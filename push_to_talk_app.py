...
class RealtimeApp(App[None]):
    ...
    async def handle_realtime_connection(self) -> None:
        async with self.client.beta.realtime.connect(model="gpt-4o-realtime-preview") as conn:
            ...
            await conn.session.update(session={"turn_detection": {"type": "server_vad"}, "voice": "echo"})
            # if you want to manually handle VAD yourself, then set `'turn_detection': None`
            # await conn.session.update(session={"tools": [weather_tool]})
            ...
            async for event in conn:
                ...
                if event.type == "response.function_call_arguments.done":
                    # Handle the completion of function call arguments
                    function_call_arguments = acc_items.pop(event.item_id, "")
                    arguments = json.loads(function_call_arguments)
                    
                    # Assuming the function to call is get_weather and it requires latitude and longitude
                    ...
                    if latitude is not None and longitude is not None:
                        ...
                        bottom_pane = self.query_one("#bottom-pane", RichLog)
                        bottom_pane.write(f"Function call completed: Temperature at ({latitude}, {longitude}) is {temperature}°C")
  
                    else:
                        bottom_pane = self.query_one("#bottom-pane", RichLog)
                        bottom_pane.write("Function call arguments are missing latitude or longitude")
                    continue
